...
def train(config):
    ...
    print ( "Training for {}".format(class_names))
    # retrieve scans
    if onevsall:
        list_images = glob.glob(root_dir + '/*/*.nii*') 
    else:
        if subclasses and parentclass: 
            list_images = [x for x in glob.glob(root_dir + '/*/*.nii*') \
...
#    for step, sample_batched in enumerate(dataloaders['train']):
#        print(step, sample_batched['image'].size(),
...
#        break
    ...
    exp_lr_scheduler = lr_scheduler.ReduceLROnPlateau(optimizer_ft, mode='max', patience=step_size, factor=gamma, verbose = True)
