...
def crop_background3D(image_np, modality='mr'):
    ...
    #         mask = output_data[x.min():x.max(), y.min():y.max(), z.min():z.max()]
        else:
...
def crop_background2D(image, modality='mr'):
    ...
    return np.float32(img), output_data
